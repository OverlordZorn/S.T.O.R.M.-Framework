...
if (_AI_presetName isEqualTo "") exitWith {"AI_Request failed - No Preset Name given"};
...
if (isNil "CVO_Storm_AI_active") then {
    if (_intensity == 0) exitWith {"AI_Request failed - No Modification currently active to be reset"};

    _default_map = [(configFile >> "CVO_AI_SubSkill_Modifier"), "CVO_AI_Skill_Default"] call cvo_storm_fnc_common_hash_from_config;

    // Is [transition active?, previous Target, current, _handler_ID]
    CVO_Storm_AI_active = [true,_default_map, _default_map, -1];
} else {
    if (CVO_Storm_AI_active#0) exitWith {"AI_Request Failed - Transition is currently in progress!"};
};
...
if (_raw_mod_Map isEqualTo false) exitWith {"AI_Request Failed - Preset not found"};

private _target_map = createHashMap;
{   _target_map set [_x, linearConversion [0,1, _intensity, 1, _y] ];   } forEach _raw_mod_map;


////////////////////////////////////////////////////////////////////////////////
// 2. pfH to Iterate during the transition: Previous Modifier -> Target Modifier
diag_log format ['[CVO](debug)(fn_AI_request) _startTime: %1 - _endTime: %2 - _raw_mod_map: %3 - _target_map: %4', _startTime , _endTime, _intensity];
...
private _codeToRun  = {
    ...
    private _current_map = createHashMap;
    {
        ...
        _target_value  = _y;
        _current_value = linearConversion [_startTime, _endTime, time, _prev_value, _target_value];
        _current_map set [_x, _current_value];            
    } forEach _target_map;
    [_current_map, (entities [["CAManBase"], [], true, true])] call CVO_STORM_fnc_AI_setSkill_recursive;
    CVO_Storm_AI_active set [2, _currentMap];
};
...
CVO_Storm_AI_active set [1,(+ _target_map)];
