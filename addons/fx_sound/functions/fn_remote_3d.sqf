...
*
* GVARS
*   GVAR(C_isActive) // key = _presetName # value =[_inInTransition, _previousIntensity, _currentIntensity, _targetIntensity]
*
*/
...
// hashmap of all active SFX spacial preset Types key = _presetName # value =[_inInTransition, _previousIntensity, _currentIntensity, _targetIntensity]
...
if (_exists) then {
    ...
    // key = _presetName # value =[_inInTransition, _previousIntensity, _currentIntensity, _targetIntensity]
    ...
    _targetIntensity = _intensity;
    GVAR(C_isActive) set [_presetName,[true, _previousIntensity, _currentIntensity, _targetIntensity] ];
...
private _exitCode = {
    private _exists = _this#2 in GVAR(C_isActive);
    if (!_exists) exitWith { ZRN_LOG_MSG_1(PFH exitCode: doesnt exist anymore,_this#2); };
    private _arr = GVAR(C_isActive) get _this#2;
    _tgtInt = _arr#3;
    _arr set [0,false];
    _arr set [1,_tgtInt];
    _arr set [2,_tgtInt];
    if (_tgtInt == 0) then {
        GVAR(C_isActive) deleteAt _this#2;
        if (count GVAR(C_isActive) == 0) then {GVAR(C_isActive) = nil};
    };
};
